      ...
      <section className="relative z-10 py-16 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 auto-rows-[200px] gap-4">
            {workItems.map((item, index) => <motion.div key={item.id} initial={{
            opacity: 0,
            y: 30
          }} whileInView={{
            opacity: 1,
            ...
            duration: 0.5
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center p-4">
                  {item.type === "video" ? <div className="w-16 h-16 rounded-full bg-primary/20 flex items-center justify-center group-hover:bg-primary/30 transition-colors mb-3">
                      <Play className="w-8 h-8 text-primary ml-1" />
                    </div> : item.type === "event" ? <Film className="w-12 h-12 text-primary/60 group-hover:text-primary transition-colors mb-3" /> : <Image className="w-12 h-12 text-primary/60 group-hover:text-primary transition-colors mb-3" />}
      ...
      <section className="relative z-10 py-20 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {services.map((service, index) => <motion.div key={service.label} initial={{
            opacity: 0,
            y: 30
          }} whileInView={{
            opacity: 1,
      ...
      <section className="relative z-10 py-20 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {behindScenes.map((item, index) => <motion.div key={item.id} initial={{
            opacity: 0,
            scale: 0.9
          }} whileInView={{
            opacity: 1,
      ...
      <section className="relative z-10 py-20 px-6 bg-gradient-to-r from-primary/5 via-transparent to-accent/5">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {productionGear.map((gear, index) => <motion.div key={gear.id} initial={{
            opacity: 0,
            y: 20
          }} whileInView={{
            opacity: 1,
